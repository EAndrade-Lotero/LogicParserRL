...
class SumTree:
    """
    SumTree data structure for Prioritized Replay Buffer - Binary Heap.
...
class PrioritizedReplayBuffer(ReplayBuffer):
    ...
    :param alpha: How much prioritization is used 
        0 - no prioritization aka uniform case
        1 - full prioritization
    :param beta: To what degree to use importance weights
        0 - no corrections
        1 - full correction
    :param final_beta: Value of beta at the end of training.
        Linear annealing is used to interpolate between initial value of beta 
        and final beta.
    :param min_priority: Minimum priority, prevents zero probabilities, so that
        all samples always have a non-zero probability to be sampled.
    """
    ...
    def sample(self, batch_size: int, env: Optional[VecNormalize] = None) -> ReplayBufferSamples:
        ...
        return self._get_samples(batch_size, env=env)
    
    def _get_samples(self, batch_size, env = None):
        """
        Sample elements from the prioritized replay buffer.
        :param batch_size: Number of element to sample
        :param batch_inds: Indices of the elements to sample.
        :param env:associated gym VecEnv
            to normalize the observations/rewards when sampling
        ...
        batch_inds = np.zeros(batch_size, dtype=np.uint32)
        ...
        for batch_idx in range(batch_size):
            ...
            batch_inds[batch_idx] = sample_idx
        ...
        return ReplayBufferSamples(*tuple(map(self.to_torch, data)), leaf_nodes_indices)
    
    # I have to see where I have to update the priorities **
    ...
    def train(self, gradient_steps: int, batch_size: int = 100) -> None:
        ...
        for _ in range(gradient_steps):
            ...
            if isinstance(self.replay_buffer, PrioritizedReplayBuffer):
                # Compute TD errors
                td_errors = th.abs(current_q_values - target_q_values)#.cpu().numpy()
                # Update priorities, they will be proportional to the td error
                self.replay_buffer.update_priorities(
                    replay_data.leaf_nodes_indices, td_errors, self._current_progress_remaining
                )
                # self.replay_buffer.update_priorities(replay_data.batch_indices, td_errors, self._current_progress_remaining)
...
class PERBuffer(ReplayBuffer):
    ...
    def _get_samples(self, batch_inds: np.ndarray, env: Optional[VecNormalize] = None) -> ReplayBufferSamples:
        """
        Adaptation to get samples based on the indices using PER formula
        """
